...
- **打包发布（2026-09-13，基线 697/697，构建前全量 pytest + doc_sync --check 双绿）**：PyInstaller 6.21.0 `delta_force_dashboard.spec --noconfirm` 重建 onedir。**避坑（构建机环境污染 → 产物混入 .NET 程序集）**：本机无项目 venv、打包走全局 Python 3.12，全局 site-packages 装了 `pyreadline3`，Windows 下 `platform` / `importlib.metadata` 的条件 import 把它拉进分析图，连带 `pyreadline3.clipboard.ironpython_clipboard` → `clr` → `pythonnet` → `clr_loader` 收集 99 个 .NET 程序集（xref 交叉引用定源）；首轮产物 375 文件 / 70.92MB、exe 7.85MB，且 UPX 对这些 DLL 全部报 `unexpected value in PE header` ——spec `excludes` 增剔 `pyreadline3/clr/pythonnet/clr_loader/psutil/yaml` 后重建，产物回落 **265 文件 / 66.02MB、exe 6.27MB**（与历史 273 文件 / 66.4MB 同档，`_internal` 仅剩 PySide6/shiboken6/pyqtgraph/numpy/cryptography/charset_normalizer/win32/pywin32_system32）。bincache 仍重定向 `PYINSTALLER_CONFIG_DIR` 至 `%TEMP%` 新目录；残留 WARNING 仅三条且均无害——`pyqtgraph.opengl` 可选子模块未收集（应用不 import）、`python3.dll` 与 `api-ms-win-core-errorhandling-l1-1-0.dll` 被 UPX 按 CFG/不可压缩跳过。**冒烟**：真实启动 PID 25524 存活 12s（172.1MB），crash.log 无新增（2184B，mtime 2026-09-01 未变），taskkill 回收后进程零残留。**release 资产**：`dist/default.zip` 重建 40.84MB（282 条目，覆盖 09-09 旧版）。**交付**：`gh release upload default --clobber` 替换 release tag `default` 的 `default.zip`（新 40.84MB / 282 条目，sha256 `290f05ab…`，覆盖 09-09 旧版 41.25MB）；`git push origin main` 推 `5d6a29d..a6c83e3`（13 提交），随后的 spec/文档提交因 github.com 被阻断改走 `gh` 的 Git Data API（见下条避坑），远端 head 仍与本地 **哈希完全一致、零分叉**。
  - **避坑（github.com:443 阻断 → gh token 走 Git Data API 推送）**：push 报 `Failed to connect to github.com:443 after 21046 ms`；`Test-NetConnection github.com -Port 443` = False，而同机 `api.github.com:443` = True、gh 命令全程正常——DNS 把 github.com 解析到 `20.205.243.166`（恰为被阻断 IP），其余 GitHub IP（`140.82.112.3`/`20.27.177.113`/`140.82.113.3` 等）均可达；本机非管理员（改不了 hosts）、无代理、无 SSH key（`~/.ssh` 仅 known_hosts）。**绕路配方**：用 `gh api` 手工推提交——`git/blobs`（内容必须取 `git cat-file blob HEAD:<path>` 的原始字节并 base64，**不要读工作区文件**：`core.autocrlf=true` 会改行尾）→ `git/trees`（`base_tree` = 远端父提交的 tree）→ `git/commits`（**message 必须带结尾 `\n`**，否则 commit 哈希不符）→ `PATCH git/refs/heads/main`（`force=false`）。每级都用本地 `git rev-parse` 的 blob/tree/commit 哈希做自校验，任一级不符即中止、不更新 ref，因此不会推错也不会产生分叉。
...
- **多主题 + 自定义主题（2026-09-09，kickoff 标准档快速档，基线 5d6a29d，6 票串行）**：
  - **方案**：预设扩展（`light`/`dark`/`nord`）+ 单槽位自定义（`custom`，base + 6 锚点继承派生），统一 `resolve_palette` 通路；现成方案判定「参考」（借 VSCode color theme 的 `include` 继承 + 语义 token 范式），零第三方依赖。
  - **交付**：`theme.py` 新增 `resolve_palette`/`register_custom`/`CUSTOM`/`ANCHOR_KEYS`/`is_hex_color`/`clean_overrides`/`contrast_hints`；`theme_dialog.py` 新增 `ThemeDialog`（base 下拉 + 6 锚点 QColorDialog + 预览 + 软提示，结果契约 `(base, overrides)`）；`sidebar.theme_btn` → QMenu（三预设 + 自定义入口）；`main_window._toggle_theme` → `_select_theme` + `_open_custom_theme_dialog`；`settings` 增 `custom_theme`（存派生源不展开，启动手改坏回退 light）。
  - **Nord 装饰色**：原版 aurora 饱和度全部 <0.55 撞 U-03 门槛，重调为「Nord 冷调提饱和」亮彩集（S min 0.588 / 对比 min 4.653 / ΔE76 min 26.0 / 亮度差 min 0.078），`NORD_BAND=(0.72,0.80)`。
  - **避坑 1（coverage source 定位）**：coverage 7.15.3 下 `--cov=app/theme` 报「无数据」（`app/__init__.py` 在 source 定位时触发 pyqtgraph/numpy 重复加载），等价替换为 `--cov=app` + `coverage report --include="app/theme.py"`。
...
- **IC-债1/2 技术债消费批次完成（2026-08-13，kickoff 全自动档，基线 `06f31df`，分支 kickoff/ic-debt）**：来源 IC 批次 code-review Standards 判断项（用户「补技术债区」指令）；Grilling 共识 D1/D2/D3 一次确认（就地捆元组 / 本地私有常量 / 最小面）；plan-tickets 落盘 `.scratch/ic-debt/`（spec + 2 工单，01=IC-债2 先行、02=IC-债1 阻塞同文件串行）；Implement 小档单智能体连续完成——01（`d6b5cd4`）`_NAV_ICON_SIZE=16` 模块级私有常量（仿 `_RENDER_DPR` 先例）apply_theme 三处显式与 icons.py 默认 size 解耦；02（`c37f5f5`）`NAV_ITEMS` 就地捆元组（`ClassVar[list[tuple[str,str]]]`）+ 删 `_NAV_ICONS` + 测试 367/2434/2435 断言同步（367 全量元组相等自带配对守卫），TDD 红→绿（对旧结构 2 failed）；**Falsify 实测**：缺图标键 1 元组 → 构造/apply_theme 两路径解包 **ValueError**（比预期 KeyError 更早更严格，快速失败达成且更强；键非法 → render_icon KeyError 消息列可用键）；`_NAV_ICON_SIZE` 改 8 → Selected pixmap 16→8 接线成立；merge `03098d5`（范围核验合规：仅 sidebar.py + test_ui_smoke.py）；全量 630/630、sidebar 99%、doc_sync 零漂移、冒烟 SMOKE OK（启动→装配→主题切换往返→关闭；踩坑：初始 light 主题 theme_btn 显示「暗色」目标，冒烟断言首次写反）；**期末四轴 0 阻断**（Standards 3：注释 KeyError/ValueError 口径偏差已修 + 无硬违规；Spec 4：两处工单验收文字口径裁决——grep「恰 3 处」实为 2 处（pixmap 折行后 size= 前缀仅 render 两行，常量裸名 5 处）、缺键异常类型 KeyError→ValueError，均工单文字问题实现优于字面；Falsify 6：zip strict=True 建议采纳；Architecture 4：无）；审核小修（注释口径 + `zip(strict=True)`）后 630/630 双绿；技术债区净清零（BD-债1~3 + IC-债1/2 全 ✅）；遥测：单 Implement 1 波（小档无波次），回退/冲突 0，空返回重开 0，四轴 findings：Standards 3 / Spec 4（2 裁决）/ Falsify 6（1 建议采纳）/ Architecture 4，阻断 0
- **知识库预检召回轨迹（IC-债1/2 消费 kickoff）**：persona（2026-08-13 版）+ 精读 0 条新增（摘要扫描相关条目：主题色 import 期冻结 / 覆盖率口径声明 / 文档漂移 / merge 残留——均无本批同款教训）；守卫反查通过（shell grep 文件名特殊字符误报，python 统一逻辑复核无缺 summary）
- **IC 批次 code-review（2026-08-13，固定点 07ecd55，双轴并行）**：Standards 2 硬违规（均低危文档失实——bonus_door_page.py:129 docstring 仍称「空操作」实调 super()；CODE_WIKI §4.22 叙述段 + §7 测试表两处「apply_theme 空操作」与已更新 sig 表自相矛盾）+ 4 baseline smell（Data Clumps：NAV_ITEMS/_NAV_ICONS 平行列表 zip 静默截断 / Magic Number：sidebar .pixmap(16,16) 与 render_icon 默认 size 耦合 / pin 取色两处散布（轻微）/ 测试耦合私有属性（白盒惯例不列））；Spec 0 阻断——1 字面偏差裁决可接受（IC-02「构造时 setIcon」→ apply_theme 单点设置，启动期 _apply_theme_refreshers 必调用，Sidebar 独立构造无图标属隐性契约）+ 1 真实缺陷（test_icons.py 断言与屏幕 DPR 耦合：icon.pixmap 在 DPR-2 屏返回 32×32，width==16 断言会失败；test_render_size_respected docstring 物理口径与断言逻辑口径矛盾）+ 1 误报已核实（§4.23 行数 ~125 与 doc_sync 非空行口径一致，子代理误数含空行总数 139）；README 60 行混合提交评估无冲突（测试数 630 实测准确）；**处置（用户「能做就做掉」偏好延续）**：4 项直接修（bonus_door_page docstring / CODE_WIKI 两处叙述 / test_icons DPR 断言改 deviceIndependentSize + 物理口径用 DPR 推导 / TO-TICKETS ADR 文件名注记 docs/adr/ADR-0006.md → 0006-svg-icons-replace-emoji.md）+ 判断项 2 条入技术债区（IC-债1 Data Clumps / IC-债2 Magic Number，均 ⚪ Speculative）；全量 630/630 + doc_sync 双绿
- **IC 批次完成 — SVG 图标替换 emoji（2026-08-13，用户拍板方案 C，ADR-0006 落档，主会话直改）**：IC-01 `app/icons.py`——`ICONS` 表内嵌 9 个 24×24 SVG 模板（ledger/wrench/key/plus/pin/moon/sun/refresh/save，Material 系填充路径）+ `render_icon(name, color, size=16)`（QSvgRenderer → 2x pixmap + DPR HiDPI；未知键 KeyError 快速失败；模块内零 get_color——C1 铁律）；`tests/test_icons.py` 6 用例（键集守卫/渲染有效/颜色注入/尺寸/未知键/占位符无残留）；IC-02 sidebar + main_window——`NAV_ITEMS` 改纯文本 + `item.setIcon`（**QIcon 双模式：Normal=FG_LABEL / Selected=accent**，选中行图标自动换色）、new_account_btn/`pin_btn` setIcon（pin 按 active 态 BTN_FG/FG_LABEL，置顶切换即时换色）、theme_btn moon/sun 图标（light→moon「暗色」/dark→sun「亮色」）、account_title 去 emoji 纯文本（标题处图标属装饰，删即替代）、`theme.py` font-family 移除 "Segoe UI Emoji"（零字体依赖）；IC-03 fetch_page_base + chart_widget——refresh_btn setIcon + **FetchPageBase 新增 apply_theme**（自动纳入 `_theme_refreshers`，C1-08 契约；**子类 CraftingPage/ExchangePage/BonusDoorPage 的 apply_theme 补 super() 调用**——契约完整性：漏补则刷新按钮图标主题切换后颜色冻结）、状态文本去 FE0F 变体（`⚠️`→`⚠`、`🔄`→`⟳`，BMP 文本符号随文字色，ADR-0006 边界）、chart 导出 action setIcon + apply_theme 重建；IC-04 `app/ui_text.py` 退役（4 处 import 清）+ U-05 守卫迁移——`test_u05_emoji_single_source` 重写为 `test_ic_emoji_free_and_icon_single_source`（**彩色 emoji/FE0F 范围正则** `[\U0001F000-\U0001FAFF\uFE0F]` 扫 app/ + 导航双态 icon 非空断言 + 文案纯文本断言；BMP 文本符号 ✓⚠⟳ 不在范围——presentation.py 同款文案语义）；新增 `test_icons_follow_theme_toggle`（主题切换后导航图标像素 == 新主题 FG_LABEL，Falsify：漏重建则颜色冻结）；IC-05 文档——CODE_WIKI §3 文件树 ui_text→icons + §3 测试列表 test_icons 行 + §4.23 icons.py 节（sig 标记）+ 4.22 apply_theme 描述更新 + README 同步（icons.py/测试数）；**全量 630/630**（623+7：icons 6 + 主题联动 1）、doc_sync 双绿；**踩坑 3 条**：① Edit 误删 class 声明（old_string 覆盖范围含 class 头，恢复——工具语义确认后重发）；② 新主题联动测试切 dark 未切回 → 全局 `theme._current_theme` 泄漏污染 test_input_panel（sample_window 不重置主题，test_theme_toggle 同款「点击切回」惯例补回）；③ 批量替换预期计数错误（"⚠️ 数据获取失败" 实际 2 处非 3——replace_in_files expected_count 守卫拦截零改动，重发修正）；README 会话期间被外部修改（icons.py 行/ADR-0006/历史叙述补录，mtime 17:37），测试数 623 中间态已同步 630；ADR-0006 落档 docs/adr/0006-svg-icons-replace-emoji.md（方案 C 取舍：颜色可控/选中态双态/HiDPI/零资源文件 vs 装配改动面）

- **打包验证（2026-08-13）**：PyInstaller 6.21.0 `delta_force_dashboard.spec --noconfirm` 重建 onedir（`dist/Delta Force Dashboard/` 67M，exe 6.8MB，UPX 压缩）；启动冒烟——真实启动 10s 进程存活（无崩溃、单实例锁正常、预加载正常）后回收；release 资产 `dist/default.zip` 重建 43.5MB（覆盖 08-12 旧版）；build/ 中间产物已生成（可随时清）

- **BD-债批次期末四轴（2026-08-13）**：固定点 `84ea3a8`，0 阻断（Standards 0 / Spec 0 / Falsify 2 判断 + 1 遗留 / Architecture 2 观察）；**评审 4 项处置**（用户「能做就做掉」偏好）：① code str `"1"` 误判业务失败（真实风险：PHP 弱类型可能返回 str）→ `code not in (1, "1")` 同化（float 1.0 经 `==` 不受影响）+ 回归测试；② `_rebuild_cards` 对 list 内 None 条目防御（BD-债3 相邻边界，`if item is None: continue`）+ 测试；③ 未知键 warning 的 int 1 / str "1" 并存重复列出 → str 化后 set 去重；④ 两 parse（OV vs bonus）业务失败语义分叉抽 `_ensure_business_ok` helper——YAGNI 观察关闭（单点不抽，未来 OV 采纳同款时归拢）；主会话直改 `0040e9a`（TDD 真红真绿 2 复现测试先行）；**全量 623/623**（621+2）、doc_sync 双绿、kkrb_parsing 100% / bonus_door_page 100%；**稳定性观察（记录不修）**：① 全量偶发 access violation 1 次（第 ~10 测试 fixture setup 崩溃，Qt teardown 时序，memory 第 7 条已知注记同款；连跑第二次全绿）；② 基线对照（stash 版）2 failed = test_doc_sync（stash 副作用的标记漂移，伪失败）+ test_input_validation_save_btn（既有 flaky，stash 版同样失败，与本次改动无关）；技术债区 BD-债1~3 归档 ✅ 净清零，评审 4 项 3 做 1 关未立新债

- **技术债批次完成 — BD-债1~3 技术债区清零（2026-08-13，kickoff 轻量档，基线 `84ea3a8`，分支 kickoff/bd-debt）**：BD-债1（业务错误码检查）`parse_bonus_door_response` code 存在且 != 1 → `KkrbError("密码门业务失败[: <msg>]")`（msg 缺省不悬挂空冒号，`{"code": 0}` → `"密码门业务失败"`；检查在 data 处理之前——业务失败优先于畸形 data 判定）；code 缺失/为 1 容错正常解析（既有无 code 畸形矩阵用例保持通过）；真红真绿（红 2 绿 4）；BD-债2（未知地图键 warning）解析前对比 data 键集与 `BONUS_DOOR_NAMES`，映射外键 `logger.warning("密码门未知地图键（需扩展 BONUS_DOOR_NAMES）: ...")`（`str(key)` 映射后排序——**Falsify 崩溃点**：非 str 键（仅手造可达）裸 sorted/join 混合类型 TypeError，修复 + 回归测试锁定），遍历逻辑不变未知键仍跳过；模块新增 `logger = logging.getLogger(__name__)`（kkrb_client/theme 同款惯例，不入 `__all__`）；真红真绿（红 1 绿 2）；BD-债3（None 字段防御）`_build_card` `item.name/password or ""`（QLabel 构造入参契约 str，纯防御）——**诚实声明：无行为级反证**（实测 PySide6 6.11.1 `QLabel(None)` 不崩、退化为空文本，C4-债6 同款），契约守卫测试锁定「None 字段 → 空串显示」；测试 614→621（+7：债1 4 / 债2 2 / 债3 1）；波内范围 **92/92**（test_kkrb_parsing 43 + test_bonus_door_page 14 + test_kkrb_client 35）；覆盖率 `kkrb_parsing` **100%** / `app/bonus_door_page` **100%**（口径注记：`--cov=app.bonus_door_page` 模块名 source 触发 numpy+coverage 已知环境冲突（BD-04 环境注记同款：cov 早期解析 import + tracer 下 C 扩展重载冲突）→ 沿用仓库既有包名口径 `--cov=app` 逐文件汇报）；doc_sync 双绿（8 标记刷新，tests_total 614→621）；技术债区 BD-债1~3 全部 ✅ 已修（净清零），活跃表清空；三轴自审：Spec 3/3 落地（消息格式/键名列表/防御点全对齐工单字面）+ Standards 0 违规（logger 惯例/__all__/docstring/无内联色） + Falsify 1 崩溃点（债2 非 str 键）已修并回归锁定；遥测：单 Implement 1 波，回退/冲突 0

- **期末四轴（BD 批次，2026-08-13）**：固定点 `4a235ca`，0 阻断——Standards 0 硬违规（安全红线 grep 干净：`password` 命中均为领域数据字段非凭据）/ Spec 0 缺失（1 处实现等效裁决：BD-02 工单字面「apply_theme get_color 运行期」→ 实现为空钩子 + QSS 选择器驱动——验收达标且为 C1-07 既有惯例）/ Falsify 3 非阻断（BD-债1 业务错误码 code:0 吞为「暂无数据」、BD-债2 未知地图键静默跳过、BD-债3 None 字段防御）/ Architecture 0 结构问题（2 观察：kkrb 协议表面线性增长 >4 端点拆传输层、NAV_ITEMS↔stack 页序索引隐式耦合为既有先例）；非阻断 → 技术债区录入 BD-债1~3；遥测：波时长 ≈42min、并行 1（单 lane）、回退/冲突 0、空返回重开 0、四轴 findings：Standards 0 / Spec 0+1 裁决 / Falsify 3 / Architecture 0+2 观察，阻断 0；技术债区净增 3（C4 全批清零后首轮累积）

- **密码门模块批次完成 — BD-01~04（2026-08-13，kickoff/bd-bonus-door，基线 `4a235ca`，来源：DESIGN_MOBILE.md v5 §5.2，桌面端第三模块）**：BD-01（`ec35780`）kkrb 数据层——`BonusDoorItem`（frozen，key/name/password/updated 全 str）+ `BONUS_DOOR_NAMES` 地图映射单源（7 键，**定义顺序即解析输出顺序**，kkrb 新增图在此扩展）+ `parse_bonus_door_response` 纯函数（畸形矩阵同 V-01：顶层非 dict 抛 KkrbError / data 非 dict → [] / 条目非 dict 跳过 / password·updated 缺省空串，`str(entry.get(x) or "")` 防 None→"None"）+ `fetch_bonus_door_data()`（`_EXCLUDED_BONUS_DOOR_KEYS = frozenset({"az3r6"})` **单点剔除**，docstring 注明两端一致硬排除策略 §5.1；复用会话/锁/60s 缓存）；FakeOpener 传输/缓存/错误降级 V-05 惯例 + 解析层不剔除 az3r6 的契约断言（排除单点归属 client）；kkrb 三文件覆盖 100%，72/72；BD-02（`6e3059c`）密码门页面——`BonusDoorPage(FetchPageBase)` **动态卡片网格**（`_render_data` 清空重建：数据量小重建成本可忽略，3→6→2 卡无残留；`_rebuild_cards` takeAt + deleteLater）；每卡地图名（`#bonusDoorMap` FG_MUTED 13px）+ 密码大字（`#bonusDoorPassword`，内联 34px bold、**颜色全 QSS 走 TEXT_PRIMARY**，C1-09 无颜色字面量守卫；**不展示更新时间** v5 拍板）；空态「暂无数据」/ 错误态「加载失败，点击重试」`_show_placeholder` 共用（AA-02 思路同 crafting `_reset_card`，C2-05 可区分）；`apply_theme` 空钩子（C1-07）；QSS 三选择器双主题由同一模板参数化天然双配，测试断言双主题各自含选择器 + TEXT_PRIMARY 双主题异值落入选择器块；`make_stub_client` 增 `bonus_impl` 参数（默认 `[]`，既有调用方零改动）；页面 100% / theme 98%，13/13（含 Falsify：`_render_data(None)` 兜底）；BD-03（`c9b30cc`）装配——侧边栏第三导航「🔑 密码门」（`EMOJI['nav_bonus_door']` 单一来源，U-05 防散落）、`QStackedWidget` 第三页（**与利润页共享同一 client 实例**，C2-02）、`_preload_profit_page` 改名 `_preload_data_pages`（扇出 profit + bonus 两页，语义不再歪斜；定时器/引用全同步）、closeEvent 加 `bonus_door_page.shutdown()`（T-01）、`app/__init__` 导出；主题刷新器自动收集断言扩展（C1-08 契约，`test_theme_refreshers_cover_all_apply_theme_widgets` 自动覆盖新页）；main_window 90% / sidebar 99% / ui_text 100%；BD-04 文档收尾——CODE_WIKI §4 补 kkrb_models（4.20）/kkrb_parsing（4.21）/bonus_door_page（4.22）三节（lines+sig 标记）+ §4.16 kkrb_client 方法表（`fetch_bonus_door_data` 等 4 行）+ §3 文件树；doc_sync 双绿、tests_total 614 与 `pytest --collect-only` 一致；TO-TICKETS BD 系列归档 + 活跃表清空；**全量 614/614**（580+34）；覆盖率口径 `--cov=app` 全量 **94%**（BD-01 kkrb 三文件 100% / BD-02 页面 100%·theme 98% / BD-03 main_window 90%·sidebar 99%）；遥测：单 Implement 串行 4 工单，回退/冲突 0；**环境注记**：coverage 7.15.3 + numpy 2.5.1 下 `--cov=app.<module>`（模块名 source）触发 numpy「cannot load module more than once per process」（`cov.start()` 早期解析 import + tracer 下 C 扩展重载冲突，预载 numpy 无效、pytest-cov 与裸 coverage 均复现）——按仓库既有口径改用包名 source `--cov=app` 测量

- **技术债批次完成 — C4-债9/11/12 技术债区清零（2026-08-13，kickoff 轻量档全自动，基线 `f70347d`）+ C4-债10 复核关闭**：C4-债9（`23817ff`）fade_in_widget duration 护栏 `max(1, duration_ms)`（motion.py:72，函数开头、`_animations_enabled` 检查之前——动效关闭路径行为不变仍返 None）——**反证实测比预测更硬**：duration=0 时 start 即 Stopped、finished 不触发、DWS 删 C++ 后 `_fade_anim` property 残留悬空 wrapper，修复前测试调返回值 `.state()` 直接 **access violation → Fatal Python error: Aborted（整个 pytest 进程 abort）**而非 RuntimeError；修复后返回有效动画 + property/effect 双收敛（断言避免 state 时序，`in (Running, Stopped)` 双值集合 = 活 wrapper 探针）；C4-债11 删 `_saved_indicator_anim` 只写句柄——grep 确认全文件零读取、无 __init__ 初始化（纯惰性创建），改直接调用不接返回值（防 GC 由 C++ parent + C4-债6 property 承担）；**契约测试必要性实证**：既有 fade 覆盖全在 motion 层（三个 U-06 测试直调 `fade_in_widget`），无一条断言经过 InputPanel 公开 API 链路——补 `test_saved_indicator_fade_contract` 填补真实缺口（text 设置 + `_fade_anim` 同步就位 + 排水收敛 + 无崩溃；`set_saved_indicator("")` 空文本分支 `if text:` 原样保留）；C4-债12 identity 测试时序加固——qWait(130) 后、identity 断言前补 `anim2.state() == Running` 辅助断言（确定性论证：QAbstractAnimation 时钟只随事件处理推进，130ms 处理 < 150ms 总时长，负载下同速延迟断言确定成立）；C4-债10 复核关闭（grep：显式 identity 比较恰 3 处 chart_widget:299/input_panel:116/kpi_presenter:263，fade property 清理集中 motion 无第二写者——「第 5 实例」触发条件未成立，零代码关闭）；全量 **580/580**（578+2）；冒烟（4.5 复核）：offscreen——duration=0/-5 护栏（触碰返回值无崩溃 + property 收敛）/ 保存指示淡入真实路径（债11）+ 排水收敛 / shake 真实链路在途销毁 + 退出无崩溃，SMOKE OK exit 0；期末四轴 0 阻断（Falsify 7 攻击面全守住：duration 0/-1/float 0.5/bool True 均收敛 1、`None` 属签名契约破坏非本批引入、动效关闭+0 无回归、空文本+在途 fade 无残留、返回值悬空为既有文档化契约边界无违规调用点）；**非阻断 3 条全为文档级微项**（docstring 补 duration 钳制语义 / 注释 C4- 前缀 / 返回值悬空契约提示）→ 直接顺手修（`92a06ce`，零行为风险，全量兜底），技术债区真正清零（净增 0）；遥测：单 Implement 1 波，回退/冲突 0，空返回重开 0，四轴 findings：Standards 2（均轻微判断）/ Spec 0（4/4 裁决通过）/ Falsify 1（非阻断提示）/ Architecture 0 结构问题，阻断 0；**技术债区（2026-08-13）清零——C4-债1~12 全批次消费完毕**
- **知识库预检召回轨迹（C4-债9~12 kickoff）**：persona（2026-08-13 版，含 Qt 自动停同 property 动画实证）+ 精读 0 条新增（复用 C4-债1~8 精读状态）；守卫反查通过；技术债区预检：C4-债9（duration 护栏）/ C4-债10（复核关闭）/ C4-债11（删只写句柄）/ C4-债12（测试加固）；工程核验：fade_in_widget duration 透传现状 + identity 检查四处 grep + `_saved_indicator_anim` 引用点 grep（仅 1 处赋值）+ identity 测试时序现状
