...
1. `app/kkrb_client.py` — 新增模块（零外部依赖，用 `urllib.request`）：
   ...
   - `fetch_cpv_data() -> list[dict]` — 调用 `POST /getCPVData`，返回战备方案数据
...
2. 数据模型（dataclass）：
   ...
   - `GearScheme` — 目标战备值、总花费、最终战备、方案列表
   - `GearItem` — 装备名、花费、战备值、来源

**影响范围**：新增 `app/kkrb_client.py`

**验收标准**：
- [ ] `fetch_ov_data()` 返回结构化数据，字段完整
- [ ] `fetch_cpv_data()` 返回结构化数据，字段完整
- [ ] 网络超时/失败时优雅降级，不阻塞 UI
...
| L-02 | kkrb.net API 客户端 — `kkrb_client.py`，封装 `getOVData`/`getCPVData` 调用 | 功能（新增） | 2026-08-05 |
| L-03 | 制造利润页面 — 4 台位推荐产物展示（台位名/产物名/利润/售价/出售时间） | 功能（新增） | 2026-08-05 |
| L-04 | 卡战备推荐页面 — 输入目标值 → 匹配档位 → 多方案展示（含装备名/磨损度/花费/战备值/来源） | 功能（新增） | 2026-08-05 |
