...
> **来源**：2026-07-30 Python Architecture Review（T 系列）+ 2026-07-31 `/improve-codebase-architecture`（C 系列）+ 2026-08-01 `收益计算器-优化建议清单.md` 评审（O 系列）+ 2026-08-02 `收益计算器项目经验复盘.md` 复盘反思（F 系列）  
...
|--------|------|------|------|----------|------|
| F-01 | 文档同步自动化：`scripts/doc_sync.py`（测试数/模块行数/方法表）+ pre-commit 防漂移钩子 | 运维（文档自动化） | 🟡 Worth exploring | 2026-08-02 | 待回填 |
| F-02 | 数据迁移「源清理时间点」策略 + `.migrated` 完成标记 | 运维（数据迁移） | ⚪ Speculative | 2026-08-02 | 待回填 |
