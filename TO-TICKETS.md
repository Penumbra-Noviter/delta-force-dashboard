...
> 活跃表（2026-08-01）：O-18（评审 `082ce62` 录入，见工单详情；O-16/O-17/O-19 已完成并归档）。此前 O-11~O-15（P2 第三批）已完成并归档，见「已完成归档」。

| Ticket | 标题 | 类型 | 优先级 | 状态 | 依赖 | 关联 |
|--------|------|------|--------|------|------|------|
| O-18 | settings.json 运行态入库清理 | 运维 | P2 | 📝 | | data.json gitignore（`95b7eef`） |
...
### O 系列（2026-08-01，O-17/O-19 批次）

| Ticket | 标题 | 类型 | 完成日期 | 提交 |
|--------|------|------|----------|------|
| O-17 | 自动清理提示文案与轮转语义不符（附显示基准统一为录入条数） | 修复 | 2026-08-01 | 待提交 |
| O-19 | CODE_WIKI 与实现失同步（方法表/依赖版本/测试计数） | 文档 | 2026-08-01 | 待提交 |
