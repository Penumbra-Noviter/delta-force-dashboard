...
| Ticket | 标题 | 类型 | 强度 |
|--------|------|------|------|
| C4-债1 | KPI 动画竞态（部分存续）：`KpiPresenter._set_kpi_value` 替换动画不停旧动画（`self._countup_anim` 引用替换但旧 QVariantAnimation 不 stop，parent 挂 presenter 防 GC 后继续跑帧）→ 数据不足直落文本 / 动效开关关闭落终态后，旧动画残留帧仍会覆盖终态（Y-05 语义失效）。账号切换场景已由 C4-02 `reset()` 终止在途动画修复。修复：赋值前 `stop()` 旧动画（复用 `app/motion.py:49-71` fade_in_widget 反竞态模式），一行可消 F-1/F-3 | 重构（竞态） | ⚪ Speculative |
