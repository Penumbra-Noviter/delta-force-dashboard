...
> 活跃表（2026-08-01）：**空**——O-18 已完成并归档。此前的 O-16/O-17/O-19 及 O-11~O-15（P2 第三批）均已完成并归档，见「已完成归档」。
...
### O-16: CSV 导出大额金额 K/M 缩写精度丢失（决策）✅

- **类型**：重构（显示一致性）/决策
- **优先级**：P2 — Worth exploring
- **文件范围**：`calculator.py`（`export_csv`）、`formatting.py`（`format_money`）
- **关联**：O-11（同一拍板）、C3（K/M/B 收敛）
- **完成日期**：2026-08-01

**问题描述**：O-11 拍板 CSV 三列统一走 `format_money`，但 `format_money` 对 ≥1e6 缩写为 `¥1,234.5K`、≥1e8 为 `¥123.5M`（`formatting.py` 阈值，K≥1e6）。CSV 作为外部分析/存档出口，大额余额导出为文本缩写、丢失全值精度，Excel 无法直接求和——与 O-11 解决思路中权衡的「数值型单元格利于后续分析」冲突（评审 `082ce62` 发现）。

**解决思路**：选项 A 保持现状（文档注明取舍）；选项 B CSV 专用全值格式化（千分位纯数值，保留两位小数）；选项 C CSV 纯数值（无千分位，最利于 Excel 计算），界面保持 `format_money`。拍板后补测试。

**拍板结论**（2026-08-01）：✅ 选 **A 保持现状**——CSV 金额列继续沿用 `format_money`，仅在 `export_csv` docstring 注明取舍（≥1e6 缩写 K/M 丢全值精度、Excel 文本不可直接求和）。B（千分位全值）/C（纯数值）留作备选，若后续需机器可读全值再启用。

### O-17: 自动清理提示文案与轮转语义不符 ✅

- **类型**：修复（文案准确性）+ 显示基准统一
- **优先级**：P3 — 顺手项
- **文件范围**：`app/main_window.py`、`calculator.py`、`tests/`
- **关联**：O-14
- **完成日期**：2026-08-01

**问题描述**：`rotate_weekly`（`calculator.py:183`）按**记录数**轮转（保留最近 7 条记录），但提示文案写「（自动清理 N 条超 7 天记录）」（`main_window.py:446`）。间断录入时（如 8 条记录跨 25 天）被删的是第 1 条（25 天前），而第 2~8 条（18 天前）均保留——「超 7 天」暗示按日历年龄删除，会误导（评审 `082ce62` 发现）。

**解决思路**：文案改为「（已保留最近 7 条记录，自动清理 N 条较早记录）」或等价记录数表述；同步改 `test_save_triggers_rotation_hint` 断言。

**实现说明**：文案改「（已保留最近 N 条记录，自动清理 N 条较早记录）」，`rotate_weekly` logger「删除超期记录」改「删除最旧记录（保留最近 %d 条）」。配套将显示基准统一为「录入条数」：`get_weekly_records`（最近 N 个日历天窗口）→ `recent_records`（最近 N 条实际录入记录），表格/图表/汇总不再按日历窗口过滤——间断录入时保留在 data 的老记录在清理前始终可见；`summary` 去掉 `end_date` 参数改用 `recent_records`，汇总标签「7日总盈亏」改「最近7条总盈亏」。测试：`get_weekly_records` 6 项改 `recent_records` 6 项（含 caps/跳过无效）+ `summary` 新增 `test_summary_caps_to_recent_days`。

### O-18: settings.json 运行态入库清理 ✅

- **类型**：运维
- **优先级**：P2 — 低风险
- **文件范围**：`.gitignore`、`settings.json`
- **关联**：data.json 已 gitignore（`95b7eef`）
- **完成日期**：2026-08-01

**问题描述**：`settings.json` 仍在 git 跟踪，但内容为运行时态（窗口几何 hash + `theme` 随主题切换翻转）。`082ce62` 已附带提交一次 light→dark 翻转，工作区现又有未提交的 dark→light + 几何变化——每次改主题/拖窗口都污染提交 diff，与 data.json 的隔离惯例不一致（评审 `082ce62` 发现）。

**解决思路**：拍板 A：`settings.json` 加入 `.gitignore` 并从索引移除（与 data.json 一致；依赖 `_load_settings` 缺省默认值兜底，O-09 已保证顶层非 dict/缺失返回默认 `{}`）；或 B：保持跟踪但提交前还原运行态。建议 A，确认后实施。

**实现说明**：选 A 实施。`.gitignore` 追加 `settings.json`（Runtime data 节，`data.json` 旁）；`git rm --cached settings.json` 出索引（磁盘文件保留）。运行态行为零变化——`_load_settings` 读磁盘、缺省/损坏返回默认 `{}`（O-09 保证）。此后主题切换/拖动窗口不再污染提交 diff。

### O-19: CODE_WIKI 与实现失同步 ✅
...
**问题描述**：`082ce62` 声称「同步 CODE_WIKI」但留下三处过期：§4.7 方法表 `rotate_weekly | days=7 | None` 与 `-> list[str]`（`calculator.py:183`）不符；§3 / §5.1 依赖 `PySide6>=6.6.0` / `pyqtgraph>=0.13.0` 与 O-12 锁定的 `==6.11.1` / `==0.14.0` 不符；§7 逐文件测试数合计 165 与 180 总数不符（评审 `082ce62` 发现）。

**解决思路**：核对源码后同步方法表签名 / 版本表 / 测试计数，一次性修正全部失实引用。

**实现说明**：以 `pytest --collect-only` 实测为准修正 §7 逐文件计数（calculator 54→57、data_store 16→18、input_panel 12→18、ui_smoke §三 23/§7.2 22→26 归一，合计 165→180），§7 覆盖范围同步至 O-08/09/11/13/14 新增用例；§3/§5.1 依赖改 `==6.11.1` / `==0.14.0` / pytest `==9.1.1`，补 `requirements-dev.txt` 行；§4.7 `rotate_weekly` 返回 `-> list[str]`（O-14）。另承接同批 O-17 重构，将 `get_weekly_records`→`recent_records`、`summary` 去 `end_date`、§一/§6.1/§7「7 日/7 天」表述统一为「最近 7 条」，与 README/PROJECT_REFERENCE 同步。
...
| O-18 | settings.json 运行态入库清理 | 运维 | 2026-08-01 | `279c311` |
