...
**当前状态**：功能完整，架构已优化。PySide6 迁移完成，第三阶段 P0-P5 + Phase 4（T-01~T-05）+ C 系列（C1~C5）+ O 系列（O-01~O-06）架构优化已完成（O-07 经评估 YAGNI 关闭），新增单实例保证、CSV 导出、今日未录入提醒、图表稀疏提示、文件日志。166 项测试全部通过。
...
- **覆盖范围**（166 项，2026-08-01）：
  ...
  - `test_ui_smoke.py`：offscreen UI 烟测（C5，承接原 verify_all；O 系列新增 CSV 导出/今日未录入/损坏设置日志/图表稀疏提示）— 23 用例
