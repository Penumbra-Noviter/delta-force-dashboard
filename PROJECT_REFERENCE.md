...
**当前状态**：功能完整，架构已优化。PySide6 迁移完成，三阶段 P0-P5 + Phase 4（T-01~T-05）+ C 系列（C1~C9）+ O 系列（O-01~O-22）架构优化全部完成（O-07 经评估 YAGNI 关闭）。187 项测试全部通过。主要能力：单实例保证、CSV 导出、今日未录入提醒、图表稀疏提示、文件日志（含轮转）、现金≤仓库不变式校验、加载结构校验、编辑态关窗确认、7 日自动删除可见性、数据目录统一到用户目录。
