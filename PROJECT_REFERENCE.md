...
**当前状态**：功能完整，架构已优化。PySide6 迁移完成，第三阶段 P0-P5 + Phase 4（T-01~T-05）+ C 系列（C1~C5）+ O 系列（O-01~O-06、O-08~O-15）架构优化已完成（O-07 经评估 YAGNI 关闭），新增单实例保证、CSV 导出、今日未录入提醒、图表稀疏提示、文件日志（含轮转）、现金≤仓库不变式校验、加载结构校验、编辑态关窗确认、7 日自动删除可见性。180 项测试全部通过。
...
- **框架**：pytest
- **覆盖范围**（180 项，2026-08-01）：
  - `calculator.py`：DayRecord 属性、增删改查、差值计算、7 日滚动/旋转、汇总、CSV 导出、不变式告警 — 55 用例
  - `data_store.py`：保存/加载回环、滚动备份编号、损坏恢复、原子写入残留、备份失败日志、顶层结构校验 — 18 用例
  - `formatting.py`：金额格式化（大/小/零/负/None）、输入解析（符号/后缀/空格/中文）、校验边界 — 58 用例
  - `test_input_panel.py`：InputPanel getter / 编辑状态归属 / save_today 走公开 API（C4 seam）/ refresh_validity 公开 seam / 不变式警告边框与保存拦截 — 18 用例
  - `test_table_theme.py`：表格主题色实时解析 + AST 防冻结（C1 回归）— 3 用例
  - `test_ui_smoke.py`：offscreen UI 烟测（C5，承接原 verify_all；O 系列新增 CSV 导出/今日未录入/损坏设置日志/图表稀疏提示/设置顶层校验）— 24 用例
...
| **打包** | PyInstaller 单文件（`dist/收益计算器.exe`），无需 hiddenimports；`app_icon.ico` 设 exe 图标 + 运行窗口图标 |
