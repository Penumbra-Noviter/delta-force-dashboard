...
# cleanup_encryption / make_store：收敛至 tests/conftest.py
...
def test_data_file_created(tmp_path):
    store = make_store(tmp_path)
    store.save({"key": "value"})
    assert (tmp_path / "data.json").exists()


def test_data_file_is_valid_json(tmp_path):
    store = make_store(tmp_path)
    store.save({"key": "value"})
...
def test_backup_rotation(tmp_path):
    store = make_store(tmp_path, max_backups=3)
    ...
    for idx in range(1, 4):
        path = tmp_path / f"data.json.bak.{idx}"
...
def test_backup_numbering(tmp_path):
    ...
    with open(tmp_path / "data.json.bak.1", "r", encoding="utf-8") as f:
        assert json.load(f) == {"step": 1}


# ── Backup failure logging ────────────────────────────

def test_rotate_backups_logs_warning_on_failure(tmp_path, caplog):
    """备份复制失败时不抛异常，仅记录 warning（不影响主流程）。"""
    store = make_store(tmp_path)
    store.save({"v": 1})  # 先落盘 data.json

    # 把备份目标指向不存在的目录 → shutil.copy2 抛 OSError
    store.backup_file = tmp_path / "missing_dir" / "data.json.bak"
...
def test_recover_from_corrupt_main(tmp_path):
    """主文件损坏时，应从最近备份恢复。"""
    store = make_store(tmp_path, max_backups=2)
    ...
    # 破坏主文件
    tmp_path.joinpath("data.json").write_text("this is not json", encoding="utf-8")
...
def test_recover_from_rolling_backup(tmp_path):
    ...
    # 破坏主文件和 .bak
    tmp_path.joinpath("data.json").write_text("corrupt", encoding="utf-8")
    tmp_path.joinpath("data.json.bak").write_text("corrupt", encoding="utf-8")
...
def test_recover_when_all_corrupt(tmp_path):
    """全部损坏时返回空字典。"""
    store = make_store(tmp_path)
    tmp_path.joinpath("data.json").write_text("bad", encoding="utf-8")
    assert store.load() == {}

def test_top_level_list_treated_as_corrupt(tmp_path):
...
def test_top_level_list_all_corrupt_returns_empty(tmp_path):
    """主文件与备份顶层都是 list 时返回空字典（O-09）。"""
    store = make_store(tmp_path)
    store.save({"ok": True})

    tmp_path.joinpath("data.json").write_text("[]", encoding="utf-8")
    tmp_path.joinpath("data.json.bak").write_text("[]", encoding="utf-8")
...
def test_no_data_file_returns_empty(tmp_path):
    store = make_store(tmp_path)
    assert store.load() == {}
...
def test_save_empty_dict(tmp_path):
    store = make_store(tmp_path)
    store.save({})
    assert store.load() == {}
...
def test_encryption_roundtrip_via_datastore(tmp_path):
...
def test_recovery_chain_intact_after_delegation(tmp_path):
...
def test_unicode_in_data(tmp_path):
    store = make_store(tmp_path)
