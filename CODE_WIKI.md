...
> 生成日期：2026-08-08  
> 测试状态：<!--AUTO:tests_total:total-->493<!--/AUTO--> 项 pytest 全部通过（含 UI 烟测 + 制造产物推荐 + 兑换利润）
...
| 测试框架 | pytest（<!--AUTO:tests_total:total-->493<!--/AUTO--> 项） |
...
│   ├── test_fetch_pages.py  ← <!--AUTO:tests:tests/test_fetch_pages.py-->18<!--/AUTO--> 个测试（T-01 FetchWorker 安全关闭/逃生舱托管 + T-02 preload 幂等/失败日志 + T-03 基类提炼回归）
...
| pytest | ==9.1.1（requirements-dev.txt） | 单元测试框架（<!--AUTO:tests_total:total-->493<!--/AUTO--> 项，含制造产物推荐 + 兑换利润） |
...
| `tests/test_fetch_pages.py` | <!--AUTO:tests:tests/test_fetch_pages.py-->18<!--/AUTO--> | T-01 FetchWorker shutdown/超时逃生舱托管/关窗不崩溃 + T-02 preload 幂等/offscreen 守卫/失败日志 + T-03 基类提炼后懒加载/渲染/主题色收敛/_error 死状态移除 |
