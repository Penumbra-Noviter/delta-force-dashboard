...
> 生成日期：2026-08-08  
> 测试状态：<!--AUTO:tests_total:total-->354<!--/AUTO--> 项 pytest 全部通过（含 UI 烟测 + 制造产物推荐 + 兑换利润）
...
| 测试框架 | pytest（<!--AUTO:tests_total:total-->354<!--/AUTO--> 项） |
...
│   ├── test_ui_smoke.py     ← <!--AUTO:tests:tests/test_ui_smoke.py-->38<!--/AUTO--> 个测试（C5 UI 烟测 + O-04/05/06/08/09/13/14，offscreen）
│   ├── test_kkrb_client.py  ← <!--AUTO:tests:tests/test_kkrb_client.py-->4<!--/AUTO--> 个测试（数据模型 + 客户端会话/传输/缓存 + 解析收敛验证）
...
│   ├── test_fetch_pages.py  ← <!--AUTO:tests:tests/test_fetch_pages.py-->14<!--/AUTO--> 个测试（T-01 FetchWorker 安全关闭/逃生舱托管 + T-02 preload 幂等/失败日志 + T-03 基类提炼回归）
...
### 4.1 `main.py` — 程序入口（<!--AUTO:lines:main.py-->~143 行<!--/AUTO-->）
...
### 4.2 `app/main_window.py` — 主窗口（<!--AUTO:lines:app/main_window.py-->~655 行<!--/AUTO-->）
...
### 4.3 `app/input_panel.py` — 输入面板（<!--AUTO:lines:app/input_panel.py-->~341 行<!--/AUTO-->）
...
### 4.4 `app/table_widget.py` — 数据表格（<!--AUTO:lines:app/table_widget.py-->~489 行<!--/AUTO-->
...
### 4.5 `app/chart_widget.py` — 图表组件（<!--AUTO:lines:app/chart_widget.py-->~540 行<!--/AUTO-->）
...
### 4.6 `app/theme.py` — 主题系统（<!--AUTO:lines:app/theme.py-->~613 行<!--/AUTO-->）
...
### 4.13 `settings_store.py` — 设置持久化（D-02，<!--AUTO:lines:settings_store.py-->~86 行<!--/AUTO-->）
...
| pytest | ==9.1.1（requirements-dev.txt） | 单元测试框架（<!--AUTO:tests_total:total-->354<!--/AUTO--> 项，含制造产物推荐 + 兑换利润） |
...
| 测试文件 | 用例数 | 覆盖范围 |
|----------|--------|----------|
| `tests/test_ui_smoke.py` | <!--AUTO:tests:tests/test_ui_smoke.py-->38<!--/AUTO--> | UI 启动/渲染、保存、编辑、删除（确认/取消）、主题切换、窗口置顶、设置持久化、几何恢复（兼容旧 Tkinter 格式）、输入校验联动（D-04 真实事件链路）、快捷键（Enter/Esc）、CSV 导出按钮、今日未录入提醒、图表稀疏提示（O-06）、编辑态关窗确认（O-13）、自动清理提示（O-14） |
| `tests/test_kkrb_client.py` | <!--AUTO:tests:tests/test_kkrb_client.py-->4<!--/AUTO--> | 数据模型 + OV 响应解析 |
| `tests/test_fetch_pages.py` | <!--AUTO:tests:tests/test_fetch_pages.py-->14<!--/AUTO--> | T-01 FetchWorker shutdown/超时逃生舱托管/关窗不崩溃 + T-02 preload 幂等/offscreen 守卫/失败日志 + T-03 基类提炼后懒加载/渲染/主题色收敛/_error 死状态移除 |
...
| `tests/test_table_theme.py` | <!--AUTO:tests:tests/test_table_theme.py-->5<!--/AUTO--> | 表格主题色实时解析（非 import 期冻结）+ AST 防复发 + D-01 零差值 |
