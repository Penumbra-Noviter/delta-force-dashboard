...
> 生成日期：2026-08-08  
> 测试状态：<!--AUTO:tests_total:total-->477<!--/AUTO--> 项 pytest 全部通过（含 UI 烟测 + 制造产物推荐 + 兑换利润）
...
| 测试框架 | pytest（<!--AUTO:tests_total:total-->477<!--/AUTO--> 项） |
...
│   ├── test_account_store.py ← <!--AUTO:tests:tests/test_account_store.py-->49<!--/AUTO--> 个测试（Y-01 多账号存储层：扫描/新建校验/resolve 兜底/DataStore 路径注入继承）
...
│   ├── test_ui_smoke.py     ← <!--AUTO:tests:tests/test_ui_smoke.py-->78<!--/AUTO--> 个测试（C5 UI 烟测 + O-04/05/06/08/09/13/14，offscreen）
...
### 4.2 `app/main_window.py` — 主窗口（<!--AUTO:lines:app/main_window.py-->~780 行<!--/AUTO-->）
...
| 方法 | 说明 |
|------|------|
| <!--AUTO:sig:app/main_window.py:MainWindow.__init__-->`__init__(store=None, logic=None, settings_store=None, registry=None, account_store=None)`<!--/AUTO--> | 加载 DataStore → 加载数据 → 初始化逻辑 → 恢复设置 → 构建 UI → 连接信号 → 应用 QSS |
...
| pytest | ==9.1.1（requirements-dev.txt） | 单元测试框架（<!--AUTO:tests_total:total-->477<!--/AUTO--> 项，含制造产物推荐 + 兑换利润） |
...
| `tests/test_account_store.py` | <!--AUTO:tests:tests/test_account_store.py-->49<!--/AUTO--> | 多账号存储层（Y-01）：list_accounts 目录扫描、create_account 校验拒绝（空/重名/禁用字符/首尾空格或点/非文本）、resolve_account 兜底回退主账号 + 空库自建、DataStore 路径注入继承（账号隔离/损坏恢复/滚动备份）、全新环境首次运行 |
