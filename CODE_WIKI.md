...
> 生成日期：2026-08-14  
> 测试状态：<!--AUTO:tests_total:total-->697<!--/AUTO--> 项 pytest 全部通过（含 UI 烟测 + 制造产物推荐 + 兑换利润）
...
| 测试框架 | pytest（<!--AUTO:tests_total:total-->697<!--/AUTO--> 项） |
...
├── tests/
│   ├── __init__.py
│   ├── conftest.py          ← pytest 共享夹具（make_stub_client 构造注入断网，C2）
│   ├── test_calculator.py
...
│   ├── test_input_panel.py  ← <!--AUTO:tests:tests/test_input_panel.py-->24<!--/AUTO--> 个测试（C4 seam + C9 静态守卫 + O-02 seam + O-08 不变式 + D-04 真实事件/焦点链路）
...
│   ├── test_ui_smoke.py     ← <!--AUTO:tests:tests/test_ui_smoke.py-->127<!--/AUTO--> 个测试（C5 UI 烟测 + O-04/05/06/08/09/13/14，offscreen）
...
│   ├── test_theme_custom.py ← <!--AUTO:tests:tests/test_theme_custom.py-->24<!--/AUTO--> 个测试（多主题 01：resolve_palette 统一取色通路 + register_custom 自定义槽位 + 三入口合并取色 + theme_guard 恢复 CUSTOM）
...
### 4.2 `app/main_window.py` — 主窗口（<!--AUTO:lines:app/main_window.py-->~703 行<!--/AUTO-->）
...
| <!--AUTO:sig:app/main_window.py:MainWindow._select_theme-->`_select_theme(name)`<!--/AUTO--> | 切换主题（预设 light/dark/nord 或已注册 custom），全链路换色并落盘（多主题 04；原 `_toggle_theme` 二值翻转移除，主题菜单经 `theme_selected` 信号接此） |
| <!--AUTO:sig:app/main_window.py:MainWindow._open_custom_theme_dialog-->`_open_custom_theme_dialog()`<!--/AUTO--> | 打开自定义主题对话框（多主题 06）：预填当前派生源，accept 后 `register_custom` + `_select_theme("custom")`，cancel 无副作用 |
...
### 4.6 `app/theme.py` — 主题系统（<!--AUTO:lines:app/theme.py-->~834 行<!--/AUTO-->）
...
| <!--AUTO:sig:app/theme.py:resolve_palette-->`resolve_palette(name)`<!--/AUTO--> | 统一取色通路（多主题 01）：内置直返 / 自定义合并 `{**THEMES[base], **overrides}` / 其余回退 `THEMES["light"]`，永不 raise；`get_color` / `generate_qss` 均经此取色 |
| <!--AUTO:sig:app/theme.py:register_custom-->`register_custom(name, base, overrides)`<!--/AUTO--> | 注册自定义主题槽位（多主题 01）：name 与内置重名或 base 非内置 → 拒绝写入 + warning（不 raise，`CUSTOM` 原状）；合法写入 `{"base", "overrides"}` |
...
### 4.7 `calculator.py` — 业务逻辑（<!--AUTO:lines:calculator.py-->~341 行<!--/AUTO-->）
...
### 4.13 `settings_store.py` — 设置持久化（D-02，<!--AUTO:lines:settings_store.py-->~111 行<!--/AUTO-->）
...
**核心**：`KpiPresenter(QObject)` 注入 4 labels（summary_label/summary_caption/cash_summary_label/cash_summary_caption），三出口——`update(logic, view_n)`（文本 + count-up 动画 + 样式全量渲染）/ `apply_theme_styles(logic, view_n)`（仅重算 signal 换色，不动文本动画，C1-08 语义）/ `reset()`（账号切换归零 + 丢弃在途动画，Y-05）。signal 计算走本模块私有 `_window_signal`（C2 深化：AA-01 单一来源落回真正的消费者内部，`update` 与 `apply_theme_styles` 共用；原先寄居 `app.main_window._kpi_signal` 并经调用期延迟导入绕循环依赖的转发已删除）。count-up 动画按磁贴独立（C4-债2 → C1 深化）：在途以磁贴 label 为注册表键（`app/motion._running`，宿主弱键），同磁贴重触发 = 丢弃旧的 + 登记新的（同目标恒单在途），跨磁贴零触碰；落值入口先 `motion.finish(label)` 优雅落终，再按**语义**条件（old≠new 且两者均非 None——C4 深化：删掉原先的「文案不等于无数据文案」判据，该文案由 presentation 产出，改文案曾可静默改变动画行为；语义上「无数据」等价于 `new is None`，已被覆盖）决定是否播新 count-up。动画生命周期（weakref 破环 / identity 检查 / 回收）全归 motion（C1），本类不持有动画对象；`reset()` 只调 `motion.stop(label)`。源码守卫：`tests/test_kpi_presenter.py::test_set_kpi_value_branches_on_semantics_not_display_text` 断言本模块不出现展示文案字面量。
...
- **C2-05 错误/空态分离 + C3 文案单源**：三态由调用路径表达——加载中/失败写 `_status_label`，成功与空态走 `_render_data(data)`，错误态走 `_render_error()`（默认实现 = 空态渲染，与既有 `_on_fetch_error` 行为逐字节等价——状态标签已承载错误提示）；占位/加载文案单源为类常量 `_EMPTY_TEXT`（「暂无数据」）/ `_ERROR_TEXT`（「加载失败，点击重试」）/ `_LOADING_TEXT`（「加载中…」，C4：状态标签加 ⟳ 前缀、ExchangePage 卡片初始占位引用同源），子类渲染引用之；CraftingPage 覆盖 `_render_error` 为错误卡片（AA-02 经 `_reset_card` 共用清空逻辑），ExchangePage 亦覆盖（C3 补齐——此前错误与空态同形，卡片显示「暂无数据」）；基类无只写不读的 `_data` 成员（C3 删除：渲染经参数传递，不靠自持状态）；
...
`BonusDoorPage(FetchPageBase)`——QStackedWidget Page 2（侧边栏第三导航「密码门」，BD-03）。网格卡片**动态构建**：`_render_data` 清空网格按数据重建（当前 6 图固定、未来可能变化，数据量小重建成本可忽略）；每卡 = 地图名（`#bonusDoorMap`，FG_MUTED）+ 密码大字（`#bonusDoorPassword`，内联 34px bold，颜色走 QSS → TEXT_PRIMARY，**不展示更新时间** v5 拍板）。空态占位 `_EMPTY_TEXT` / 错误态占位 `_ERROR_TEXT`（C2-05 可区分，C3 文案单源至基类常量，`_show_placeholder` 共用）。`apply_theme` 仅刷新基类刷新按钮图标（IC-03，super()；卡片颜色全 QSS 选择器驱动，C1-07）；MainWindow 启动预加载与 closeEvent 回收（`_preload_data_pages` 单出口）。

| 方法 | 说明 |
|------|------|
| <!--AUTO:sig:app/bonus_door_page.py:BonusDoorPage._fetch-->`_fetch()`<!--/AUTO--> | 后台线程取数：`client.fetch_bonus_door_data()`（az3r6 已在 client 层剔除） |
| <!--AUTO:sig:app/bonus_door_page.py:BonusDoorPage._render_data-->`_render_data(data)`<!--/AUTO--> | 清空卡片网格按数据重建；空数据 → 占位 `_EMPTY_TEXT` |
| <!--AUTO:sig:app/bonus_door_page.py:BonusDoorPage._render_error-->`_render_error()`<!--/AUTO--> | 错误态：清空卡片 + 占位 `_ERROR_TEXT`（与空态可区分） |
| <!--AUTO:sig:app/bonus_door_page.py:BonusDoorPage.apply_theme-->`apply_theme()`<!--/AUTO--> | 主题切换钩子：super() 刷新基类刷新按钮图标（IC-03；卡片颜色全 QSS 选择器驱动） |

---

### 4.23 `app/icons.py` — SVG 矢量图标（IC 系列，ADR-0006，<!--AUTO:lines:app/icons.py-->~125 行<!--/AUTO-->）

替代 U-05 emoji 装饰图标的单色矢量图标模块（用户拍板方案 C）。`ICONS` 表内嵌 9 个 24×24 viewBox SVG 模板（ledger/wrench/key/plus/pin/moon/sun/refresh/save，Material 系填充路径，风格统一）；`render_icon(name, color, size=16)` 经 QSvgRenderer → QPixmap（2x 物理像素 + devicePixelRatio，HiDPI）→ QIcon。**颜色由调用方注入**（运行期 `get_color`，模块内零 get_color 调用——C1 铁律）；未知键 raise KeyError（编译期常量，快速失败）。主题联动：各落点 `apply_theme` 重建图标（sidebar 导航双态色 Normal=FG_LABEL/Selected=accent、pin active=BTN_FG；main_window theme_btn moon/sun；fetch_page_base refresh_btn；chart_widget 导出菜单）。`app/ui_text.py` 随本批次退役；BMP 文本符号（✓⚠⟳）保留为文案语义。

| 方法 | 说明 |
|------|------|
| <!--AUTO:sig:app/icons.py:render_icon-->`render_icon(name, color, size=16)`<!--/AUTO--> | 按注入色渲染命名图标为 QIcon（2x + DPR，HiDPI）；未知键 KeyError |
...
### 4.27 `app/theme_dialog.py` — 自定义主题对话框（多主题 05，<!--AUTO:lines:app/theme_dialog.py-->~139 行<!--/AUTO-->）
...
| pytest | ==9.1.1（requirements-dev.txt） | 单元测试框架（<!--AUTO:tests_total:total-->697<!--/AUTO--> 项，含制造产物推荐 + 兑换利润） |
...
| `tests/test_theme_custom.py` | <!--AUTO:tests:tests/test_theme_custom.py-->24<!--/AUTO--> | 多主题（01）：resolve_palette 内置直返/自定义合并 base+overrides/未知回退 light/畸形槽位永不 raise；register_custom 合法写入与非法拒绝（name 重名/非内置 base，不 raise、CUSTOM 原状）；set_theme 接受 THEMES∪CUSTOM；get_color/generate_qss/signal_color/summary_style 经 resolve_palette 取色；theme_guard 恢复 CUSTOM 槽位 |
...
| 测试文件 | 用例数 | 覆盖范围 |
|----------|--------|----------|
| `tests/test_ui_smoke.py` | <!--AUTO:tests:tests/test_ui_smoke.py-->127<!--/AUTO--> | UI 启动/渲染、保存、编辑、删除（确认/取消）、主题切换、窗口置顶、设置持久化、几何恢复（兼容旧 Tkinter 格式）、输入校验联动（D-04 真实事件链路）、快捷键（Enter/Esc）、CSV 导出按钮、今日未录入提醒、图表稀疏提示（O-06）、编辑态关窗确认（O-13）、自动清理提示（O-14）；Y 系列账号（Y-03 解析链路/兜底/落盘回读、Y-04 账号区初始态/新建/非法名拒绝/注入隐藏、Y-05 切换刷新/重启回读/编辑复用态取消/保存删除 CSV 落新账号/同账号 no-op） |
...
> **技术唯一来源**（F-01）：本文档为技术细节权威源；Serena memory 的「关键避坑」为快速摘要，细节以本文档为准（单向权威，勿双源维护）。
